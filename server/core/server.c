             ...
             char *authentication_options)
{
    ...
    {
        /** Load default authenticator for this protocol */
        GWPROTOCOL *protofuncs = (GWPROTOCOL*)load_module(protocol, MODULE_PROTOCOL);

        if (protofuncs == NULL)
        {
            MXS_ERROR("Failed to load protocol module '%s' for server at %s:%u.",
                      protocol, servname, port);
            return NULL;
        }
        else if (protofuncs->auth_default == NULL)
        {
            MXS_ERROR("No authenticator configured for server at %s:%u and protocol "
                      "'%s' does not define a default authenticator.",
                      servname, port, protocol);
            return NULL;
        }

        authenticator = protofuncs->auth_default();
    }
